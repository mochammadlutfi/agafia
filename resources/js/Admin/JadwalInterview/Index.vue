    ...
    <base-layout>
        <div class="content">
            <div class="content-heading d-flex justify-content-between align-items-center">
                ...
                <div class="space-x-1">
                    <el-button type="primary" :tag="Link" :href="route('admin.interview.jadwal.create')">
                            ...
                            <template #default="scope">
                                <el-dropdown popper-class="dropdown-action" trigger="click">
                                    ...
                                    <template #dropdown>
                                        <el-dropdown-menu>
                                            <el-dropdown-item>
                                              <Link :href="route('admin.interview.jadwal.show', {id: scope.row.id})" class="w-100 d-flex align-items-center justify-content-between space-x-1">
                                                Detail
                                                <i class="si fa-fw si-eye"></i>
                                              </Link>
                                            </el-dropdown-item>
                                            <el-dropdown-item>
                                              <Link :href="route('admin.interview.jadwal.edit', {id: scope.row.id})" class="w-100 d-flex align-items-center justify-content-between space-x-1">
                                                Ubah
...
const fetchData = async (pg) => {
  try {
    isLoading.value = true;
    const response = await axios.get('/admin/interview/jadwal/data', {
      params: {
        page: pg || 1,
        limit,
...
const hapus = (id) => {
  ElMessageBox.alert('Data yang dihapus tidak bisa dikembalikan!', 'Peringatan', {
    ...
    router.delete(`/admin/interview/jadwal/${id}/delete`, {
      preserveScroll: true,
      onSuccess: () => {
        fetchData();
        ElMessage({
